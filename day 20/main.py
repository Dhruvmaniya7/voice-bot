...
# --- Gemini function that streams to Murf ---
async def get_llm_response_stream(transcript: str):
    ...
    try:
        async with websockets.connect(murf_uri) as websocket:
            ...
            async def receive_and_print_audio():
                chunk_count = 1
                while True:
                    try:
            ...
            receiver_task = asyncio.create_task(receive_and_print_audio())
...
async def websocket_audio_streaming(websocket: WebSocket):
    ...
    if not config.ASSEMBLYAI_API_KEY:
        ...
        await websocket.send_text(json.dumps({"type": "error", "message": "AssemblyAI API key not configured on the server."}))
    ...
    def on_turn(self: Type[StreamingClient], event: TurnEvent):
        ...
        if event.end_of_turn and event.turn_is_formatted:
            ...
            asyncio.run_coroutine_threadsafe(get_llm_response_stream(transcript_text), main_loop)
