...
async def get_llm_response_stream(transcript: str, client_websocket: WebSocket):
    ...
    try:
        async with websockets.connect(murf_uri) as websocket:
            voice_id = "en-US-diva" # Hardcoded female voice
            ...
            try:
                # Changed persona name to Diva
                prompt = f"""You are Diva, a friendly and conversational AI voice assistant.
A user is speaking to you and has just said: "{transcript}"

Your response should be concise, natural, and friendly, as if you were speaking.
IMPORTANT: Do not use any markdown formatting like asterisks or bullet points. Provide only the plain text for your response.
"""
...
async def websocket_audio_streaming(websocket: WebSocket):
    ...
    def on_turn(self: Type[StreamingClient], event: TurnEvent):
        ...
        if event.end_of_turn and event.turn_is_formatted and transcript_text and transcript_text != last_processed_transcript:
            ...
            llm_task = asyncio.run_coroutine_threadsafe(get_llm_response_stream(transcript_text, websocket), main_loop)
